...
*****************************************************************************************
修改者：tao
修改：OSInit() 和 OS_Sched() 和 OSIntExit() 和 INT16U等类型声明改为unsigned int等
*****************************************************************************************
*/
...
*                            units are unsigned longs then, the stack contains '4 * stk_size' bytes.  The stack
...
INT8U OS_TCBInit(INT8U prio, OS_STK *ptos, OS_STK *pbos, unsigned int id, unsigned long stk_size, void *pext, unsigned int opt)
{
#if OS_CRITICAL_METHOD == 3 /* Allocate storage for CPU status register */
    OS_CPU_SR cpu_sr;
#endif
    ...
    if (ptcb != (OS_TCB *)0)
    {
        ...
        OSTCBInitHook(ptcb);
#endif
        //我们没有用户自定义的事件 OS_VERSION改为低于204也是这个道理
