...
INT8U OSTimeDlyHMSM(INT8U hours, INT8U minutes, INT8U seconds, unsigned int milli)
{
    unsigned long ticks;
    unsigned int loops;
    ...
    if (hours > 0 || minutes > 0 || seconds > 0 || milli > 0)
    {
        ...
        ticks = ((unsigned long)hours * 3600L + (unsigned long)minutes * 60L + (unsigned long)seconds) * OS_TICKS_PER_SEC + OS_TICKS_PER_SEC * ((unsigned long)milli + 500L / OS_TICKS_PER_SEC) / 1000L;
        loops = (unsigned int)(ticks / 65536L); /* Compute the integral number of 65536 tick delays   */
        ticks = ticks % 65536L;           /* Obtain  the fractional number of ticks             */
        OSTimeDly((unsigned int)ticks);
        while (loops > 0)
        {
            OSTimeDly(32768);
            OSTimeDly(32768);
...
#if OS_TIME_GET_SET_EN > 0
void OSTimeSet(unsigned long ticks)
{
#if OS_CRITICAL_METHOD == 3 /* Allocate storage for CPU status register           */
    OS_CPU_SR cpu_sr;
#endif
